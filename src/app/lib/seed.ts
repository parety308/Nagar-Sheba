...
const DEPARTMENTS = [
  {
    ...
    description:
      "Potholes, road damage, streetlights, footpaths",
  },
  {
    name: "Waste Management",
    description:
      "Garbage collection, illegal dumping, bin repairs",
  },
  {
    name: "Water Supply",
    description:
      "Water leakage, supply interruptions, drainage",
  },
  {
    name: "Licensing & Permits",
    description:
      "Trade licences, noise permits, tree-cutting permits",
  },
] as const;
...
export const seed = async (): Promise<void> => {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    for (const category of CATEGORIES) {
      const departmentId = departmentIdByName.get(
        category.department,
      );

      if (!departmentId) {
        throw new Error(
          `Department not found: ${category.department}`,
        );
      }
    ...
    for (const staff of STAFF) {
      ...
      if (!departmentId) {
        throw new Error(
          `Department not found: ${staff.department}`,
        );
      }
